...
| D | Ship + flight plan (plan-and-burn) | B, C | 1 | Flight plan as ordered quantized BurnNodes on the projected trajectory (spec: flight-plan-model-v0.1.md; supersedes the original commit-and-burn row per the Overseer's 2026-08-07 ruling, longburn-wci/wbu). Executed burns are append-only events; planned burns are paper, edited/aborted via atomic PlanRevision commands that travel at c from HQ and are validated at ARRIVAL with typed refusal events. **No event type can un-execute a burn** (pillar 2 in the types); phases (docked→accel→coast→flip→decel→arrived; zero coast for torch, impulsive for Hohmann) are derived views, not authoritative state |
...
| G | Notification layer | D | 2 | Desktop/web push on: transfer window opening, light-lagged ship reports (burn executed, revision applied/refused — all at c), last-revision-instant warnings for upcoming burns, arrival, market events worth waking for. §6 calls the notification layer the retention mechanic, so it is treated as product, not plumbing |
...
| H2 | Client: planning + commitment UI | H1, D, F | 2 | Tradeoff-surface picker (porkchop-style) exposing the full torch-to-Hohmann continuum as the economic/tactical/fuel decision; the commit ritual; in-flight replanning UI (KSP maneuver-node reference — planning is a local player tool, never light-lagged; the ORDER carries the lag, and the UI shows the last instant a revision could still arrive before each burn); market panel with staleness indicators |
