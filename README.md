...
`HostTickDriver` is the live host boundary: it samples wall-clock milliseconds on a configured interval and passes only elapsed milliseconds into `AuthoritativeSimLoop`. Its clock and timer are injectable for deterministic host tests; neither enters `src/sim/`.
