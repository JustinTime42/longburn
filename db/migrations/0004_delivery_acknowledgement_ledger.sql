-- Supersede 0003's single-watermark cursor before durable deployments exist.
-- Re-runnable by design: no production cursor data exists to preserve yet.
DROP TABLE IF EXISTS delivery_acknowledgements;
DROP TABLE IF EXISTS delivery_assignments;
DROP TABLE IF EXISTS delivery_cursors;

CREATE TABLE delivery_cursors (
  observer_id TEXT PRIMARY KEY CHECK (length(observer_id) > 0),
  low_watermark BIGINT NOT NULL DEFAULT 0 CHECK (low_watermark >= 0),
  next_delivery_sequence BIGINT NOT NULL DEFAULT 1 CHECK (next_delivery_sequence > 0)
);

-- Assignment is a durable fact made at first projection. It remains after its
-- acknowledgement receipt compacts so future projections look up, never rank.
CREATE TABLE delivery_assignments (
  observer_id TEXT NOT NULL,
  delivery_sequence BIGINT NOT NULL CHECK (delivery_sequence > 0),
  message_id TEXT NOT NULL CHECK (length(message_id) > 0),
  source_global_position BIGINT NOT NULL CHECK (source_global_position > 0),
  PRIMARY KEY (observer_id, delivery_sequence),
  UNIQUE (observer_id, message_id)
);

CREATE TABLE delivery_acknowledgements (
  observer_id TEXT NOT NULL,
  delivery_sequence BIGINT NOT NULL CHECK (delivery_sequence > 0),
  PRIMARY KEY (observer_id, delivery_sequence),
  FOREIGN KEY (observer_id, delivery_sequence)
    REFERENCES delivery_assignments (observer_id, delivery_sequence)
);
