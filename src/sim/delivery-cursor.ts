...
export interface DeliveryAcknowledgement {
  ...
  /** Immutable source position of the durable event that produced this message. */
...
export interface DeliveryCursor {
  ...
  /** Highest immutable source position ever acknowledged for this observer. */
  readonly acknowledgedThroughPosition: number;
...
const validAcknowledgement = (acknowledgement: DeliveryAcknowledgement): void => {
  if (acknowledgement.messageId.length === 0 || !Number.isSafeInteger(acknowledgement.deliverySequence) || acknowledgement.deliverySequence < 1 ||
    !Number.isSafeInteger(acknowledgement.sourceGlobalPosition) || acknowledgement.sourceGlobalPosition < 1) {
    throw new RangeError("Delivery acknowledgements require a non-empty message ID, positive delivery sequence, and positive source global position.");
  }
};
...
const normalize = (observerId: string, lowWatermark: number, acknowledgedThroughPosition: number, delivered: readonly DeliveryAcknowledgement[]): DeliveryCursor => {
  if (observerId.length === 0 || !Number.isSafeInteger(lowWatermark) || lowWatermark < 0 ||
    !Number.isSafeInteger(acknowledgedThroughPosition) || acknowledgedThroughPosition < 0) {
    throw new RangeError("Delivery cursors require a non-empty observer ID and non-negative watermarks.");
  }
  ...
  return { observerId, lowWatermark: compactedWatermark, acknowledgedThroughPosition, delivered: [...bySequence.values()].sort((a, b) => a.deliverySequence - b.deliverySequence) };
};
...
export class InMemoryDeliveryCursorStore implements DeliveryCursorStore {
  ...
  async acknowledge(observerId: string, acknowledgement: DeliveryAcknowledgement): Promise<DeliveryCursor> {
    validAcknowledgement(acknowledgement);
    const previous = this.#cursors.get(observerId) ?? { observerId, lowWatermark: 0, acknowledgedThroughPosition: 0, delivered: [] };
    if (hasAcknowledged(previous, acknowledgement)) throw new RangeError("Delivery message is already acknowledged.");
    const cursor = normalize(observerId, previous.lowWatermark, Math.max(previous.acknowledgedThroughPosition, acknowledgement.sourceGlobalPosition), [...previous.delivered, acknowledgement]);
...
interface CursorRow extends Record<string, unknown> {
  ...
  readonly acknowledged_through_position: number;
  readonly delivery_sequence: number | null;
  readonly message_id: string | null;
  readonly source_global_position: number | null;
}
  ...
  async read(observerId: string): Promise<DeliveryCursor | undefined> {
    const result = await this.#client.query<CursorRow>(
      `SELECT c.observer_id, c.low_watermark::double precision AS low_watermark,
              c.acknowledged_through_position::double precision AS acknowledged_through_position,
              a.delivery_sequence::double precision AS delivery_sequence, a.message_id,
              a.source_global_position::double precision AS source_global_position
  ...
  async acknowledge(observerId: string, acknowledgement: DeliveryAcknowledgement): Promise<DeliveryCursor> {
    ...
    return this.#client.withTransaction(async (session) => {
      const inserted = await session.query<{ readonly observer_id: string }>(
        `INSERT INTO delivery_acknowledgements (observer_id, delivery_sequence, message_id, source_global_position)
         VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING RETURNING observer_id`,
        [observerId, acknowledgement.deliverySequence, acknowledgement.messageId, acknowledgement.sourceGlobalPosition]
      );
      if (inserted.rows.length === 0) throw new RangeError("Delivery message is already acknowledged.");

      await session.query(
        `INSERT INTO delivery_cursors (observer_id, low_watermark, acknowledged_through_position)
         VALUES ($1, 0, 0) ON CONFLICT DO NOTHING`, [observerId]
      );
      ...
      await session.query(
        "UPDATE delivery_cursors SET low_watermark = $2, acknowledged_through_position = GREATEST(acknowledged_through_position, $3) WHERE observer_id = $1",
        [observerId, beforeCompaction.lowWatermark, acknowledgement.sourceGlobalPosition]
      );
      await session.query(
...
const deserializeCursor = (rows: readonly CursorRow[]): DeliveryCursor => {
  ...
  const delivered = rows.flatMap((row) => row.delivery_sequence === null || row.message_id === null || row.source_global_position === null
    ? [] : [{ deliverySequence: row.delivery_sequence, messageId: row.message_id, sourceGlobalPosition: row.source_global_position }]);
  return normalize(first.observer_id, first.low_watermark, first.acknowledged_through_position, delivered);
};
